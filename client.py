...
def apply_camera_settings(settings):
    if settings:
        ...
        print(f"Camera settings applied: {width}x{height} @ {fps} FPS")
...
def start_recording():
    ...
    if not is_recording:
        ...
        recording_file_path = os.path.join(
            output_dir, f"recording_{timestamp}_{args.video}_.mp4")
        timestamp_file_path = os.path.join(output_dir, f"timestamps_{
                                           timestamp}__{args.video}.txt")
        timestamps = []
...
def stop_recording():
    ...
    if is_recording:
        ...
        print(f"Recording stopped and saved to {recording_file_path}")
...
def upload_video_and_timestamps(video_path, timestamp_path):
    with open(video_path, 'rb') as video_file, open(timestamp_path, 'rb') as timestamp_file:
        files = {
        ...
        if response.status_code == 200:
            print(f"Video and timestamps uploaded successfully.")
        else:
            print(f"Failed to upload files. Status code: {
